...
const InputFile = ({ setImageUrl }: IInputFile) => {
    ...
    return (
        <>
            <div
                className={`border-dashed flex justify-center items-center border-2 rounded transition relative w-full h-full  ${!imagePreview && 'cursor-pointer hover:bg-orange-50'
                    }`}
            ...
            >
                {imagePreview ? (
                    <div className="absolute h-full">
                        <Image width={800} height={400} src={imagePreview} alt="" className="h-full" />
                    ...
                    <span className='text-gray-400 text-sm'>Click to upload or drag and drop SVG, PNG, JPG or GIF </span>
                )}
            </div>
            <input
